...
if (corpus.schemaVersion !== manifest.corpus.version || !Array.isArray(corpus.cases) || corpus.cases.length === 0) fail("invalid corpus envelope");
...
for (const testCase of corpus.cases) {
  ...
  if (!testCase.grammar.startsWith("Examples/Corpus/") || !testCase.grammar.endsWith(".grammar")) fail(`invalid grammar path for ${testCase.id}`);
  if (!existsSync(join(root, testCase.grammar))) fail(`missing grammar for ${testCase.id}`);
...
if (cliIndex >= 0) {
  ...
  try {
    for (const testCase of corpus.cases) {
      ...
      const result = spawnSync(resolve(cli), ["parse", join(root, testCase.grammar), testCase.input, output], { encoding: "utf8" });
...
console.log(`Ecosystem contract valid: ${manifest.repositories.length} pinned repositories, ${corpus.cases.length} corpus cases${cliIndex >= 0 ? ", Workbench conformant" : ""}.`);
