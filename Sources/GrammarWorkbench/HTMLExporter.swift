...
enum HTMLExporter {
    static func render(_ artifact: GrammarArtifact) -> String {
        ...
        let decisions = artifact.decisions.map { "<li><strong>\(escape($0.title))</strong><br>Witness: <code>\(escape($0.witness.joined(separator: " ")))</code><p>\(escape($0.explanation))</p></li>" }.joined()
        return """
        <!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>Grammar Artifact</title>
        <style>body{font:15px system-ui;margin:auto;max-width:1100px;padding:32px;color:#172033}pre,code{font-family:ui-monospace,monospace}main{display:grid;grid-template-columns:1fr 1fr;gap:20px}section{border:1px solid #ccd3df;border-radius:10px;padding:12px}table{border-collapse:collapse;width:100%}td,th{border:1px solid #ccd3df;padding:7px}.conflict{background:#ffd9d5;color:#8d1710}@media(max-width:700px){main{grid-template-columns:1fr}}</style></head>
        <body><h1>Grammar Artifact Explorer</h1><p>Algorithm: <strong>\(escape(artifact.algorithm.rawValue))</strong></p><h2>Grammar</h2><pre>\(escape(artifact.grammarSource))</pre><h2>Automaton</h2>\(automaton)<h2>Parsing table</h2><table><tr><th>State</th>\((artifact.terminals + artifact.nonterminals).map { "<th>\(escape($0))</th>" }.joined())</tr>\(rows)</table><h2>Conflicts and decisions</h2><ol>\(decisions)</ol><h2>Sample parse</h2><p><code>\(escape(artifact.sample.input))</code></p><pre>\(escape(artifact.sample.tree))</pre></body></html>
        """
    }
