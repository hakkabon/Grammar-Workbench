...
public enum GrammarLexerRuntime {
    ...
    public static func lex(_ source: String, grammar: ParsedGrammar) -> LexerResult {
        var rules: [CompiledRule] = grammar.lexerRules.enumerated().compactMap { offset, rule in
            ...
            return CompiledRule(token: rule.token, expression: expression, order: offset)
        }
        ...
        for terminal in grammar.terminals where terminal != "$" && !patternedTokens.contains(terminal) {
            ...
            rules.append(.init(token: terminal, expression: expression, order: rules.count))
        }
        ...
        while location < text.length {
            ...
            guard let best else {
                ...
                diagnostics.append(.init(id: diagnostics.count, message: "No lexer rule matches ‘\(unexpected)’.", range: range))
                location += length
                continue
            }
        ...
        return LexerResult(source: source, tokens: output, diagnostics: diagnostics)
    }
